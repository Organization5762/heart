...
sudo mkdir -p \
  "${DKMS_ROOT}/rust/heart_rust/kernel/pi5_scan_loop" \
  "${DKMS_ROOT}/rust/heart_rust/native"

sudo install -m 0644 \
  "${REPO_ROOT}/rust/heart_rust/kernel/pi5_scan_loop/Makefile" \
...
sudo install -m 0644 \
  ...
  "${DKMS_ROOT}/rust/heart_rust/kernel/pi5_scan_loop/heart_pi5_scan_loop.c"
sudo install -m 0644 \
  "${REPO_ROOT}/rust/heart_rust/native/pi5_scan_loop_ioctl.h" \
  "${DKMS_ROOT}/rust/heart_rust/native/pi5_scan_loop_ioctl.h"
