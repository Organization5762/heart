...
The Game of Life update in `heart.renderers.modules.life.state.LifeState` relied on a convolution-based neighbor count. This note captures the alternative padding-based neighbor count used to reduce per-frame overhead while keeping the same update rules.

## Materials

- `src/heart/renderers/modules/life/state.py` for the update logic and neighbor counting.
...
- Neighbor counting logic: `heart.renderers.modules.life.state._count_neighbors_with_padding`.
- Strategy selection: `heart.renderers.modules.life.state.LifeState._update_grid`.
