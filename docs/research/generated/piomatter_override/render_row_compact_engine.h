...
inline std::vector<uint32_t> build_compact_row_words(
...
void protomatter_render_rgb10(std::vector<uint32_t> &result,
    ...
    for (size_t addr = 0; addr < n_addr; addr++) {
        for (auto &schedule_ent : sched) {
            ...
            auto compact_row_words = build_compact_row_words(row_words, active_hold_count);
            frame_words.insert(frame_words.end(), compact_row_words.begin(), compact_row_words.end());
