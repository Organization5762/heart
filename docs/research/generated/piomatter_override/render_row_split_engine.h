...
constexpr uint32_t command_kind_shift = 31;
constexpr uint32_t command_trailer_shift = 30;
constexpr uint32_t command_row_repeat = 0;
constexpr uint32_t command_row_split = 1u << command_kind_shift;
constexpr uint32_t command_counted_trailer = 1u << command_trailer_shift;
...
inline uint32_t encode_repeat_row_command(size_t logical_count, bool counted_trailer) {
    assert(logical_count >= 1);
    assert(logical_count < (size_t{1} << command_trailer_shift));
    return command_row_repeat | (counted_trailer ? command_counted_trailer : 0u)
           | uint32_t(logical_count - 1);
}

inline uint32_t encode_split_row_command(size_t logical_count, bool counted_trailer) {
    assert(logical_count >= 1);
    assert(logical_count < (size_t{1} << command_trailer_shift));
    return command_row_split | (counted_trailer ? command_counted_trailer : 0u)
           | uint32_t(logical_count - 1);
}
...
inline std::vector<uint32_t> build_split_repeat_row_words(
    std::span<const uint32_t> row_words, bool counted_trailer
) {
    assert(!row_words.empty());

    auto spans = split_row_if_two_runs(row_words);
    assert(spans.has_value());

    if (spans->second_count == 0) {
        return {
            encode_repeat_row_command(spans->first_count, counted_trailer),
            spans->first_word,
        };
    }

    return {
        encode_split_row_command(spans->first_count, counted_trailer),
        spans->first_word,
...
void protomatter_render_rgb10(std::vector<uint32_t> &result,
    ...
    for (size_t addr = 0; addr < n_addr; addr++) {
        for (auto &schedule_ent : sched) {
            ...
            const bool counted_trailer = active_hold_count != 0;

            auto split_row_words = build_split_repeat_row_words(row_words, counted_trailer);
            frame_words.insert(frame_words.end(), split_row_words.begin(), split_row_words.end());
            if (counted_trailer) {
