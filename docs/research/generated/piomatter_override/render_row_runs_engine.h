...
constexpr uint32_t command_extra_runs_shift = 30;
constexpr uint32_t command_first_width_shift = 22;
constexpr uint32_t command_first_width_bits = 8;
constexpr uint32_t command_active_hold_limit = (uint32_t{1} << command_first_width_shift) - 1;
constexpr uint32_t command_first_width_limit = (uint32_t{1} << command_first_width_bits) - 1;
constexpr uint32_t command_extra_runs_limit = 3;
...
inline uint32_t encode_runs_row_command(
    size_t first_logical_count, size_t extra_run_count, uint32_t active_hold_count
) {
    assert(first_logical_count >= 1);
    assert(first_logical_count - 1 <= command_first_width_limit);
    assert(extra_run_count <= command_extra_runs_limit);
    assert(active_hold_count <= command_active_hold_limit);
    return (uint32_t(extra_run_count) << command_extra_runs_shift)
           | (uint32_t(first_logical_count - 1) << command_first_width_shift) | active_hold_count;
}

inline std::optional<std::vector<row_run>> split_row_into_runs(
    std::span<const uint32_t> row_words, size_t max_runs
) {
    if (row_words.empty()) {
        return std::nullopt;
    }
    ...
    runs.reserve(max_runs);
    uint32_t current_word = row_words.front();
    ...
    for (uint32_t word : row_words) {
        ...
        runs.push_back(row_run{current_count, current_word});
...
inline std::vector<uint32_t> build_runs_row_words(
    std::span<const uint32_t> row_words, uint32_t active_hold_count
) {
    constexpr size_t max_runs = 4;
    auto runs = split_row_into_runs(row_words, max_runs);
    assert(runs.has_value());

    std::vector<uint32_t> encoded_words;
    encoded_words.reserve(2 + (runs->size() - 1) * 2);
    encoded_words.push_back(
        encode_runs_row_command(runs->front().logical_count, runs->size() - 1, active_hold_count)
    );
    encoded_words.push_back(runs->front().word);
    for (size_t run_index = 1; run_index < runs->size(); run_index++) {
        encoded_words.push_back(uint32_t(runs->at(run_index).logical_count - 1));
        encoded_words.push_back(runs->at(run_index).word);
    }
...
void protomatter_render_rgb10(std::vector<uint32_t> &result,
    ...
    for (size_t addr = 0; addr < n_addr; addr++) {
        for (auto &schedule_ent : sched) {
            ...
            auto runs_row_words = build_runs_row_words(row_words, active_hold_count);
            frame_words.insert(frame_words.end(), runs_row_words.begin(), runs_row_words.end());
