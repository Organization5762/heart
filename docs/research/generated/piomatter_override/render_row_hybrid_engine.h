...
constexpr uint32_t command_active_hold_shift = 8;
constexpr uint32_t command_width_bits = 8;
constexpr uint32_t command_active_hold_limit =
    (uint32_t{1} << (command_kind_shift - command_width_bits)) - 1;
constexpr uint32_t command_width_limit = (uint32_t{1} << command_width_bits) - 1;
...
inline uint32_t encode_literal_row_command(size_t logical_count, uint32_t active_hold_count) {
    assert(logical_count >= 1);
    assert(logical_count - 1 <= command_width_limit);
    assert(active_hold_count <= command_active_hold_limit);
    return command_row_literal | (active_hold_count << command_active_hold_shift)
           | uint32_t(logical_count - 1);
}

inline uint32_t encode_repeat_row_command(size_t logical_count, uint32_t active_hold_count) {
    assert(logical_count >= 1);
    assert(logical_count - 1 <= command_width_limit);
    assert(active_hold_count <= command_active_hold_limit);
    return command_row_repeat | (active_hold_count << command_active_hold_shift)
           | uint32_t(logical_count - 1);
}

inline uint32_t encode_split_row_command(size_t first_count, uint32_t active_hold_count) {
    assert(first_count >= 1);
    assert(first_count - 1 <= command_width_limit);
    assert(active_hold_count <= command_active_hold_limit);
    return command_row_split | (active_hold_count << command_active_hold_shift)
...
inline std::vector<uint32_t> build_hybrid_row_words(
    std::span<const uint32_t> row_words, uint32_t active_hold_count
) {
    ...
    if (std::all_of(
            row_words.begin(),
            row_words.end(),
            [&](uint32_t word) { return word == row_words.front(); })) {
        return {
            encode_repeat_row_command(row_words.size(), active_hold_count),
            row_words.front(),
        };
    }

    if (auto spans = split_row_if_two_runs(row_words); spans.has_value() && spans->second_count != 0) {
        return {
            encode_split_row_command(spans->first_count, active_hold_count),
            spans->first_word,
            uint32_t(spans->second_count - 1),
            spans->second_word,
        };
    }
    ...
    literal_words.push_back(encode_literal_row_command(row_words.size(), active_hold_count));
...
void protomatter_render_rgb10(std::vector<uint32_t> &result,
    ...
    for (size_t addr = 0; addr < n_addr; addr++) {
        for (auto &schedule_ent : sched) {
            ...
            auto hybrid_row_words = build_hybrid_row_words(row_words, active_hold_count);
            frame_words.insert(frame_words.end(), hybrid_row_words.begin(), hybrid_row_words.end());
