...
constexpr uint32_t command_row_literal = 0;
constexpr uint32_t command_row_repeat = 1u << command_kind_shift;
constexpr uint32_t command_row_split = 2u << command_kind_shift;
constexpr uint32_t command_row_window = 3u << command_kind_shift;
...
inline uint32_t encode_literal_row_command(size_t logical_count) {
    assert(logical_count >= 1);
    assert(logical_count < (size_t{1} << command_kind_shift));
    return command_row_literal | uint32_t(logical_count - 1);
}

inline uint32_t encode_repeat_row_command(size_t logical_count) {
    assert(logical_count >= 1);
    assert(logical_count < (size_t{1} << command_kind_shift));
    return command_row_repeat | uint32_t(logical_count - 1);
}

inline uint32_t encode_split_row_command(size_t first_logical_count) {
    assert(first_logical_count >= 1);
    assert(first_logical_count < (size_t{1} << command_kind_shift));
    return command_row_split | uint32_t(first_logical_count - 1);
}

inline uint32_t encode_window_row_command(size_t first_logical_count) {
    assert(first_logical_count >= 1);
    assert(first_logical_count < (size_t{1} << command_kind_shift));
    return command_row_window | uint32_t(first_logical_count - 1);
}

inline std::optional<split_row_spans> split_row_if_two_runs(std::span<const uint32_t> row_words) {
    if (row_words.empty()) {
        return std::nullopt;
    }

    const uint32_t first_word = row_words.front();
    size_t split_index = 0;
    while (split_index < row_words.size() && row_words[split_index] == first_word) {
        split_index++;
    }

    if (split_index == row_words.size()) {
        return split_row_spans{row_words.size(), first_word, 0, first_word};
    }

    const uint32_t second_word = row_words[split_index];
    for (size_t index = split_index; index < row_words.size(); index++) {
        if (row_words[index] != second_word) {
            return std::nullopt;
        }
    }

    return split_row_spans{
        split_index,
        first_word,
        row_words.size() - split_index,
        second_word,
    };
}

inline std::optional<window_row_spans> split_row_if_window(std::span<const uint32_t> row_words) {
    if (row_words.empty()) {
        return std::nullopt;
    }

    const uint32_t edge_word = row_words.front();
    size_t first_count = 0;
    while (first_count < row_words.size() && row_words[first_count] == edge_word) {
        first_count++;
    }

    if (first_count == row_words.size()) {
        return window_row_spans{row_words.size(), edge_word, 0, edge_word, 0};
    }

    size_t tail_start = row_words.size();
    while (tail_start > first_count && row_words[tail_start - 1] == edge_word) {
        tail_start--;
    }
    const size_t tail_count = row_words.size() - tail_start;
    if (tail_count == 0) {
        return std::nullopt;
    }

    const uint32_t middle_word = row_words[first_count];
    for (size_t index = first_count; index < tail_start; index++) {
        if (row_words[index] != middle_word) {
            return std::nullopt;
        }
    }

    return window_row_spans{
        first_count,
        edge_word,
...
inline std::vector<uint32_t> build_window_row_words(std::span<const uint32_t> row_words) {
    assert(!row_words.empty());

    auto spans = split_row_if_window(row_words);
    if (spans.has_value() && spans->middle_count == 0 && spans->tail_count == 0) {
        return {
            encode_repeat_row_command(spans->first_count),
            spans->edge_word,
        };
    }
    ...
    if (spans.has_value()) {
        return {
            encode_window_row_command(spans->first_count),
            spans->edge_word,
            uint32_t(spans->middle_count - 1),
            spans->middle_word,
            uint32_t(spans->tail_count - 1),
        };
    }

    if (auto split_spans = split_row_if_two_runs(row_words);
        split_spans.has_value() && split_spans->second_count != 0) {
        return {
            encode_split_row_command(split_spans->first_count),
            split_spans->first_word,
            uint32_t(split_spans->second_count - 1),
            split_spans->second_word,
        };
    }

    std::vector<uint32_t> literal_words;
    literal_words.reserve(1 + row_words.size());
    literal_words.push_back(encode_literal_row_command(row_words.size()));
    literal_words.insert(literal_words.end(), row_words.begin(), row_words.end());
...
void protomatter_render_rgb10(std::vector<uint32_t> &result,
    ...
    for (size_t addr = 0; addr < n_addr; addr++) {
        for (auto &schedule_ent : sched) {
            ...
            const uint32_t next_addr_word = calc_addr_bits(addr) | pinout::oe_inactive;
            const uint32_t active_hold_count = encode_delay_count(
                active_time * CLOCKS_PER_DATA / CLOCKS_PER_DELAY - DELAY_OVERHEAD
            );

            auto encoded_row_words = build_window_row_words(row_words);
            frame_words.insert(frame_words.end(), encoded_row_words.begin(), encoded_row_words.end());
            frame_words.push_back(active_hold_count);
            ...
            frame_words.push_back(next_addr_word);
        }
    }
