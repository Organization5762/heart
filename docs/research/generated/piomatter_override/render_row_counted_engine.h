...
inline std::vector<uint32_t> build_counted_row_words(std::span<const uint32_t> row_words) {
    ...
    if (std::all_of(
        ...
        return {
            encode_repeat_row_command(row_words.size()),
            row_words.front(),
        };
    }
...
void protomatter_render_rgb10(std::vector<uint32_t> &result,
    ...
    for (size_t addr = 0; addr < n_addr; addr++) {
        for (auto &schedule_ent : sched) {
            ...
            auto counted_row_words = build_counted_row_words(row_words);
            frame_words.insert(frame_words.end(), counted_row_words.begin(), counted_row_words.end());
