...
- Host running the `heart` runtime with access to `src/heart/display/renderers/` and `src/heart/peripheral/` packages.
...
- [ ] Introduce an `OutputChannel` interface in `src/heart/display/renderers/__init__.py` with `submit_frame(surface, metadata)` and `submit_statistics(stats)` methods.
- [ ] Implement `FrameStatisticsSampler` in `src/heart/display/color.py` to compute average RGB and luminance.
- [ ] Add a `StatisticsDispatchRenderer` that wraps existing renderers in `src/heart/display/renderers/multi_scene.py`, capturing post-render hooks.
