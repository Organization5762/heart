...
export function PhoneControlPanel() {
  ...
  const handlePhotoZoomChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const nextZoom = Number(event.target.value);
    setPhotoDraft((current) =>
      current ? normalizePhotoDraft({ ...current, cropZoom: nextZoom }) : current,
    );
  };
        ...
        <div className="mt-4 grid gap-3 sm:mt-5">
          <div className="beats-console-card rounded-2xl px-4 py-4">
            ...
            <div className="mt-2 text-sm leading-6 text-[#95a2b6]">
              Send free text to the totem over Wi-Fi using the existing Beats socket.
            </div>
            <label className="mt-3 block">
              ...
              <textarea
                ...
                rows={4}
                className="min-h-28 w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-base leading-6 text-slate-100 outline-none transition placeholder:text-slate-400/70 focus:border-emerald-300/40 focus:ring-2 focus:ring-emerald-300/20 disabled:cursor-not-allowed disabled:opacity-45"
              />
            </label>
            <div className="mt-2 flex items-center justify-between gap-3 text-xs text-[#95a2b6]">
              <span>{textDraft.length}/{TEXT_CHARACTER_LIMIT} characters</span>
              <span>{trimmedTextDraft ? "Ready to send" : "Enter text to enable send"}</span>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-3">
              <button
                ...
                className="flex min-h-14 items-center justify-center gap-2 rounded-[1.1rem] border border-emerald-300/30 bg-emerald-400/12 px-4 py-3 font-tomorrow text-sm tracking-[0.12em] text-emerald-50 uppercase transition active:translate-y-[1px] disabled:cursor-not-allowed disabled:opacity-45 hover:bg-emerald-400/18"
              >
              ...
              <button
                ...
                className="flex min-h-14 items-center justify-center gap-2 rounded-[1.1rem] border border-white/12 bg-white/5 px-4 py-3 font-tomorrow text-sm tracking-[0.12em] text-slate-100 uppercase transition active:translate-y-[1px] disabled:cursor-not-allowed disabled:opacity-45 hover:bg-white/10"
              >
                <X className="h-4 w-4" />
                Clear
              </button>
          ...
          <div className="beats-console-card rounded-2xl px-4 py-4">
            ...
            <div className="mt-2 text-sm leading-6 text-[#95a2b6]">
              Open the camera on your phone, preview the shot, then push it to the
              totem using the same full-screen fitted image path as `photo_only`.
            </div>
            <div className="mt-3 grid grid-cols-2 gap-3">
              <button
                type="button"
                ...
                className="flex min-h-14 items-center justify-center gap-2 rounded-[1.1rem] border border-sky-300/28 bg-sky-400/10 px-4 py-3 font-tomorrow text-sm tracking-[0.12em] text-sky-50 uppercase transition active:translate-y-[1px] disabled:cursor-not-allowed disabled:opacity-45 hover:bg-sky-400/16"
              >
            ...
            {photoDraft ? (
              <div
                ref={cropFrameRef}
                className={cn(
                  "relative mt-3 aspect-square overflow-hidden rounded-2xl bg-black/30 select-none touch-none",
            ...
            {photoDraft ? (
              <div className="mt-2 text-xs text-[#95a2b6]">
                Source {photoDraft.sourceWidth}x{photoDraft.sourceHeight}, exported as a
                square {PHOTO_OUTPUT_SIZE}x{PHOTO_OUTPUT_SIZE} crop.
              </div>
            ) : null}
            {photoDraft ? (
              <button
                ...
                className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-[1.1rem] border border-white/12 bg-white/5 px-4 py-3 font-tomorrow text-sm tracking-[0.12em] text-slate-100 uppercase transition active:translate-y-[1px] hover:bg-white/10"
              >
                <X className="h-4 w-4" />
                Remove Photo
              </button>
...
function StatusCard({
  ...
  return (
    <div className="beats-console-card rounded-2xl px-4 py-3">
      ...
      <div
        className={cn(
          "mt-2 break-words font-mono text-lg text-slate-100",
...
function getPhotoPreviewMetrics(photoDraft: PhotoDraft): {
  ...
  return {
    renderedHeightPercent: photoDraft.sourceHeight * coverScale * photoDraft.cropZoom,
    renderedWidthPercent: photoDraft.sourceWidth * coverScale * photoDraft.cropZoom,
  };
}
...
async function renderPhotoCrop(photoDraft: PhotoDraft): Promise<{
  ...
  editorContext.drawImage(image, offsetX, offsetY, renderedWidth, renderedHeight);

  const cropInset = Math.round(
    (editorSize - PHOTO_OUTPUT_SIZE) / 2,
  );
  ...
  const blob = await canvasToBlob(outputCanvas, "image/jpeg", PHOTO_JPEG_QUALITY);
  return {
