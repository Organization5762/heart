...
VCC3V3 = Net("3V3")
VCC3V3.drive = POWER
VCORE_1V2 = Net("VCORE_1V2")
VCORE_1V2.drive = POWER
GND = Net("GND")
GND.drive = POWER
...
class BoardConfig:
    ...
    def buffer_part(self) -> Part:
        ...
        return Part(
            "74HCT245BQ_115", "74HCT245BQ,115", footprint="Package_SO:74HCT245BQ115"
        )
...
class Hub75DriverBoard:
    ...
    def __init__(self, cfg: BoardConfig | None = None):
        ...
        self._create_fpga()
        self.allocator = FPGAPinAllocator(fpga=self.fpga)
    ...
    def _create_fpga(self):
        self.fpga = Part(
            self.cfg.SNAP_EDA, self.cfg.FGPA_NAME, footprint=self.cfg.FPGA_FOOTPRINT
        )
        self.fpga_io_pins = self._gather_io_pins()

    def _gather_io_pins(self):
        logger.info(
            "FPGA pin function histogram: %s", Counter([p.func for p in self.fpga.pins])
        )
    ...
    def _create_res_array(self, idx):
        rn = Part(
            "Device",
            self.cfg.RESARRAY_NAME,
            value="33",
            footprint=self.cfg.RESARRAY_FOOT,
        )
        rn.ref = f"RN{idx}"
        return rn
    ...
    def _create_hub75_connector(self, idx):
        hub = Part(
            "Connector_Generic",
            "Conn_02x08_Odd_Even",
            footprint=self.cfg.CONNECTOR_FOOT,
        )
        hub.ref = f"J{idx}"
        return hub
    ...
    def _wire_fpga_to_buffers(self, port_idx, bufs, sig):
        """Fan HUB75 nets through two 74HCT244 buffers.
...
class FPGAPinAllocator:
    ...
    def allocate_region(
        self, *, col_size=4, row_size=5, start_at: tuple[str, int] | None = None
    ) -> list[list[str]]:
        all_valid = []
        ...
        raise RuntimeError(
            f"No {row_size}×{col_size} IO region left on the FPGA. Ran out on board {self.counter}"
        )
