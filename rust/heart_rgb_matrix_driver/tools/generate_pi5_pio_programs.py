...
C_HEADER_PATH = (
    ROOT_DIR / "rust" / "heart_rgb_matrix_driver" / "native" / "generated" / "pi5_pio_programs_generated.h"
)
RUST_OUTPUT_PATH = (
    ROOT_DIR
    / "rust"
...
def render_c_header(programs: tuple[AssembledProgram, ...]) -> str:
    lines = [
    ...
    for program in programs:
        prefix = program.spec.const_prefix
        lines.extend(
            [
                f"#define {prefix}_PROGRAM_LENGTH {len(program.instructions)}u",
                f"#define {prefix}_WRAP_TARGET {program.wrap_target}u",
                f"#define {prefix}_WRAP {program.wrap}u",
                f"#define {prefix}_SIDESET_PIN_COUNT {program.sideset_pin_count}u",
                f"#define {prefix}_SIDESET_OPTIONAL {1 if program.sideset_optional else 0}",
                f"#define {prefix}_SIDESET_TOTAL_BITS {program.sideset_pin_count + int(program.sideset_optional)}u",
                f"#define {prefix}_OUT_PIN_BASE {program.spec.out_pin_base}u",
                f"#define {prefix}_OUT_PIN_COUNT {program.spec.out_pin_count}u",
                f"#define {prefix}_OUT_SHIFT_RIGHT {1 if program.spec.out_shift_right else 0}",
                f"#define {prefix}_AUTO_PULL {1 if program.spec.auto_pull else 0}",
                f"#define {prefix}_PULL_THRESHOLD {program.spec.pull_threshold}u",
                "",
                f"static const uint16_t {prefix.lower()}_base_program[{len(program.instructions)}] = {{",
                format_instruction_array(program.instructions, "u"),
                "};",
                "",
            ]
        )
        if program.spec.delay_patch_indices:
            patch_indices = ", ".join(str(index) for index in program.spec.delay_patch_indices)
            lines.append(
                f"static const uint8_t {prefix.lower()}_delay_patch_indices[{len(program.spec.delay_patch_indices)}] = {{{patch_indices}}};"
            )
            lines.append("")
    lines.append("#endif")
    lines.append("")
...
def main() -> int:
    ...
    c_header = render_c_header(programs)
    rust_module = render_rust_module(programs)

    changed_paths = []
    for path, content in ((C_HEADER_PATH, c_header), (RUST_OUTPUT_PATH, rust_module)):
