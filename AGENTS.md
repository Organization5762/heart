...
- `2026-03-31`: Gated `pyo3-stub-gen` behind a new `stubgen` Cargo feature and marked `src/bin/stub_gen.rs` with `required-features = ["stubgen"]`. This keeps Pi-side `pi5_scan_bench` builds from compiling the unused stub-generation parser stack, which had been crashing `cargo run --release --bin pi5_scan_bench` on `michael@totem1.local` inside `rustpython-parser`. Local validation: `cargo check --manifest-path rust/heart_rust/Cargo.toml --bin pi5_scan_bench` and `cargo check --manifest-path rust/heart_rust/Cargo.toml --features stubgen --bin stub_gen`.
- `2026-03-31`: Shortened the shared Pi 5 scan parser from 31 to 25 instructions by internalizing the trailer control path: `OE` now rides on sideset together with the clock, `LAT` is pulsed internally via `SET PINS`, and the packed trailer now carries only the end-of-spans marker plus dwell. Local validation: `cargo check --manifest-path rust/heart_rust/Cargo.toml --bin pi5_scan_bench`, `cargo test --manifest-path rust/heart_rust/Cargo.toml pi5_scan_pack_rgba`, `make format`, and `make test`.
...
- `2026-03-31`: Tightened the Pi 5 resident-loop worker concurrency model: the worker now waits on a single `worker_runnable` flag with `wait_event()`, snapshots replay batches under the session lock, checks stop/disable state inside the batch loop, records replay timing/counter telemetry, binds the kthread to CPU 0, and runs it with `SCHED_FIFO` priority `80` via `sched_setattr_nocheck()`. Local validation: `cargo check --manifest-path rust/heart_rust/Cargo.toml --bin pi5_scan_bench`, `cargo test --manifest-path rust/heart_rust/Cargo.toml pi5_scan_pack_rgba`, `make format`, and `make test`.
