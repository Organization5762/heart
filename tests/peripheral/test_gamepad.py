...
from heart.peripheral.gamepad.gamepad import (
    BLUETOOTH_RECONNECT_INTERVAL_SECONDS, DEFAULT_BLUETOOTH_GAMEPAD_MACS,
    Gamepad, GamepadIdentifier)
from heart.peripheral.gamepad.peripheral_mappings import SwitchProMapping


class TestGamepadMapping:
    def test_short_switch_pro_name_uses_switch_pro_mapping(self) -> None:
        gamepad = Gamepad(
            joystick=SimpleNamespace(
                get_name=lambda: GamepadIdentifier.SWITCH_PRO_SHORT.value,
            )
        )

        mapping = GamepadController._mapping_for_gamepad(gamepad)

        assert isinstance(mapping, SwitchProMapping)
