...
class TestFirmwareIoRotaryEncoderHandler:
        ...
        [
            (
                ...
                [
                    (
                        "initial",
                        [rotary_encoder.form_json(constants.SWITCH_ROTATION, 0, 7)],
                    ),
                    (
                        "rotate-forward",
                        [rotary_encoder.form_json(constants.SWITCH_ROTATION, 4, 7)],
                    ),
                    ("steady", []),
                ],
            ),
            (
                ...
                [
                    (
                        "initial",
                        [rotary_encoder.form_json(constants.SWITCH_ROTATION, 0, 5)],
                    ),
                    (
                        "rotate-negative",
                        [rotary_encoder.form_json(constants.SWITCH_ROTATION, -3, 5)],
                    ),
                    ("repeat", []),
                ],
            ),
    ...
    def test_handle_emits_rotation_events_when_position_changes(
        self, rotary_components, index, steps, expected
    ) -> None:
    ...
    def test_handle_emits_button_press_for_press_release_sequence(
        ...
        switch = SimpleNamespace(
            value=released_value, pull=getattr(dummy_pull, pull_direction)
        )
        ...
        timeline = run_handler_steps(
            handler, encoder, switch, steps, clock=deterministic_clock
        )
    ...
    def test_handle_emits_long_press_once_when_threshold_elapsed(
        self, dummy_pull, deterministic_clock, advance_sequence
    ) -> None:
        ...
        expected_long_press = rotary_encoder.form_json(
            constants.BUTTON_LONG_PRESS, 1, 1
        )
        labels = [
            "baseline",
            "press-start",
