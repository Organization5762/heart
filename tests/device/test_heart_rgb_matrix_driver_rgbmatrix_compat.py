"""Tests for the opt-in heart_rgb_matrix_driver rgbmatrix compatibility layer."""
...
from heart.device.rgb_display import heart_rgb_matrix_driver_rgbmatrix
...
class FakeNativeMatrixDriver:
    ...
    def stats(self) -> object:
        return SimpleNamespace(backend_name="fake-heart-rgb-matrix-driver")

    def close(self) -> None:
        self.closed = True


class TestHeartRgbMatrixDriverRgbmatrixCompatibility:
    """Validate the separate heart_rgb_matrix_driver rgbmatrix shim so legacy-style callers can opt in without touching the real rgbmatrix import path."""
    ...
    def test_rgb_matrix_options_translate_supported_geometry_and_color(
        ...
        """Verify the compatibility layer maps supported rgbmatrix options into the clean-room native config. This matters because migration code should keep its geometry and color-order intent when moving onto the heart_rgb_matrix_driver device stack."""
    ...
    def test_rgb_matrix_proxies_canvas_swap_and_clear(
        ...
        """Verify the compatibility wrapper forwards the familiar canvas and clear calls to the native driver. This matters because existing draw loops should be able to reuse their CreateFrameCanvas/SwapOnVSync structure on top of heart_rgb_matrix_driver."""
        ...
        matrix = heart_rgb_matrix_driver_rgbmatrix.RGBMatrix()
    ...
    def test_rgb_matrix_warns_when_legacy_options_are_ignored(
        ...
        monkeypatch.setattr(
            heart_rgb_matrix_driver_rgbmatrix.logger,
            "warning",
