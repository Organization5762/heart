...
- Panel is connected directly to the LED/HUB75 output on `totem4`.
- Saleae Logic is intentionally skipped for the current loop.
...
- The `logic2` connector is available, but fresh capture is currently blocked
  by an already-recording Saleae session (`Cannot switch sessions while recording`).
...
shared frame buffer at about 62 Hz. `dwell88` is the best measured balance so
far: much brighter than the first RGB333 pass, but still over the aggregate
1200 fps target.
...
(
  ...
  sleep 1
  sudo ./rp1_hub75_cycle_rgb333 0xc000 28 16
...
There is now a true multi-bit PWM path. RGB333 stores 3 bits per R/G/B channel
for both upper and lower pixels in each row-pair word. RP1 expands it into a
7-pass repeated bit schedule: bit0 once, bit1 twice, bit2 four times. This gives
512 possible colors per pixel instead of the previous 8-color RGB111 mode.
