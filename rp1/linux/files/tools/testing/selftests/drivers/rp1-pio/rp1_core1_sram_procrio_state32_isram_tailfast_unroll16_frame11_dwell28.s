...
.include "rp1_core1_hub75_gpio_pins.inc"
.ifndef DWELL_ITERS
.equ DWELL_ITERS,  28
.endif
...
.macro RGB_CLEAR_SETTLE_DELAY
.ifdef RGB_CLEAR_SETTLE_NOPS
    CONFIG_NOPS RGB_CLEAR_SETTLE_NOPS
.endif
.endm
...
.ifdef USE_RGB_SETCLR_EXPLICIT_CLKLOW
    str r5, [r10]
    RGB_CLEAR_SETTLE_DELAY
    str \reg, [r8]
    PRECLK_SETUP_DELAY
    str r9, [r8]
.ifdef USE_RGB_SETCLR_CLK_HIGH_CALL
    bl rgb_setclr_clk_high_delay
.else
.ifdef RGB_SETCLR_CLK_HIGH_LOOP_ITERS
    movs r1, #RGB_SETCLR_CLK_HIGH_LOOP_ITERS
.Lrgb_setclr_clk_high_loop\@:
    subs r1, #1
    bne .Lrgb_setclr_clk_high_loop\@
.else
    CLK_HIGH_DELAY
.endif
.endif
...
.endif
    ...
    movw r1, #:lower16:DMA_CHUNK_DATA_BASE
    movt r1, #:upper16:DMA_CHUNK_DATA_BASE
...
.ifdef USE_STATE32_DMA_PIPELINE_WRAP_PREFETCH
    /* Optional latency optimization: prefetch the next frame before the current
     * frame has fully drained. Keep disabled for artifact-sensitive scanners
     * because it can make frame-boundary SRAM/DMA contention visible. */
...
.else
    /* No wrap prefetch: all in-flight chunks have been displayed, so restart
     * at frame setup and issue a fresh initial DMA fetch there. */
