...
class AvatarBpmStateProvider(StateProvider[AvatarBpmRendererState]):
    def states(
        self, peripheral_manager: PeripheralManager
    ) -> Subscribable[AvatarBpmRendererState]:
        return (
    ...
    def _select_top_bpm(self) -> AvatarBpmRendererState:
        ...
        try:
            active_bpms = [(addr, bpm) for addr, bpm in current_bpms.items() if bpm > 0]
        except ValueError:
            return AvatarBpmRendererState(sensor_id=None, bpm=None, avatar_name=None)
        if not active_bpms:
            return AvatarBpmRendererState(sensor_id=None, bpm=None, avatar_name=None)
