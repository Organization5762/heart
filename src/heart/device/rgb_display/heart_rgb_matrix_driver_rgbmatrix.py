"""Opt-in rgbmatrix-style compatibility layer backed by the native matrix driver.
...
clean-room ``heart_rgb_matrix_driver`` driver stack.
...
logged when callers override them, but they are not applied by the native driver.
"""
...
class RGBMatrix:
    """rgbmatrix-style wrapper that submits frames through ``heart_rgb_matrix_driver``."""
...
def _log_ignored_option_overrides(options: RGBMatrixOptions) -> None:
    ...
    if ignored_overrides:
        logger.warning(
            "heart_rgb_matrix_driver RGBMatrix compatibility ignores legacy rgbmatrix options "
