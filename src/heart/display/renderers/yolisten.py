    ...
    def process(
        ...
        if should_show:
            ...
            for i, word in enumerate(words_to_show):
                ...
                if word == "Y'HEAR":
                    total_lines = (
                        len(self.ascii_art[word][0]) + len(self.ascii_art[word][1]) + 1
                    )  # 1 for spacing
                else:
                    total_lines = len(self.ascii_art[word])
                y_offset = (
                    window_height - (total_lines * (self.ascii_font_sizes[word] + 1))
                ) // 2
                ...
                self._draw_ascii_art_with_x_offset(
                    word, y_offset, screen_surface, x_offset_accel
                )
