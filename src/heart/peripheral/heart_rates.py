...
class HeartRateManager(Peripheral):
    ...
    def __init__(self) -> None:
        ...
        self._lock = asyncio.Lock()  # Protect access to shared state
        logger.info("HeartRateManager initialized")
    ...
    async def _manage_connection(self, device: BLEDevice) -> None:
        ...
        try:
            await client.connect(timeout=20.0)  # Increased timeout
    ...
    async def _scan_and_manage(self) -> None:
        ...
        scanner = BleakScanner(service_uuids=[HEART_RATE_SERVICE_UUID])
        scanner.register_detection_callback(self._handle_detection)
    ...
    async def _handle_detection(self, device: BLEDevice, advertisement_data):
        ...
        async with self._lock:
            if (
                ...
                logger.info(
                    f"Found new HR device {device.address}, attempting to manage."
                )
                task = asyncio.create_task(self._manage_connection(device))
                self._managing_tasks[device.address] = task
            # else: logger.debug(f"Device {device.address} already known.")
    ...
    def run(self) -> None:
        ...
        logger.info("Starting HeartRateManager run loop...")
