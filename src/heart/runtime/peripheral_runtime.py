...
BEATS_STREAM_INPUT_DEBUG_ENV_VAR = "BEATS_STREAM_INPUT_DEBUG"
...
class PeripheralRuntime:
    ...
    def configure_streaming(self, websocket: Any | None = None) -> None:
        ...
        if websocket is not None or _should_stream_input_debug():
            ...
            logger.debug(
                "Beats input debug streaming disabled; set %s=1 to forward debug tap events",
                BEATS_STREAM_INPUT_DEBUG_ENV_VAR,
            )
...
def _should_stream_input_debug() -> bool:
    return _env_flag(BEATS_STREAM_INPUT_DEBUG_ENV_VAR, default=False)
