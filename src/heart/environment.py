...
class GameLoop:
    def __init__(self, device: Device, peripherial_manager: PeripherialManager, max_fps: int = 60) -> None:
        self.initalized = False
        self.device = device
        self.peripherial_manager = peripherial_manager
    ...
    def active_mode(self) -> GameMode:
        mode_index = self.peripherial_manager._deprecated_get_main_switch().get_button_value() % len(self.modes)
    ...
    def process_renderer(self, renderer: "BaseRenderer") -> Image.Image | None:
        try:
            ...
            renderer.process(
                window=screen,
                clock=self.clock,
                peripherial_manager=self.peripherial_manager
            )
    ...
    def _initialize(self) -> None:
        ...
        logger.info("Attempting to detect attached peripherials")
        self.peripherial_manager.detect()
        logger.info(f"Detected attached peripherials - found {len(self.peripherial_manager.peripherials)}. {self.peripherial_manager.peripherials=}")
    ...
    def __process_debugging_key_presses(self):
        ...
        switch = self.peripherial_manager._deprecated_get_main_switch()
        payload = None
