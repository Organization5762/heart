...
if ! slot_meta_matches_scanner_pwm "${slot_meta}"; then
  log "refusing scanner PWM mismatch: scanner pwm=${RP1_SCANNER_PWM_BITS} expected slot meta=${RP1_EXPECTED_SLOT_META} observed slot meta=${slot_meta}. Set HEART_RP1_HUB75_PWM_BITS and HEART_PI5_SIMPLE_SCAN_DEFAULT_PWM_BITS to the same value."
  exit 4
fi
