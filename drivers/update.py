...
def load_driver_libs(libs: list[str], destination: str) -> None:
    ...
    if not os.path.isdir(destination):
        raise NotADirectoryError(f"The destination path '{destination}' is not a valid directory.")
    
    print(f"Loading the following libs: {libs}")
    zip_location = download_file(CIRCUIT_PY_COMMON_LIBS, CIRCUIT_PY_COMMON_LIBS_CHECKSUM)
    # Lib in this case just coems from the downloaded file, which is separate from the `destination` which is also lib
    lib_path = os.path.join(zip_location.replace(".zip", ""), "lib")

    if not os.path.exists(lib_path):
        subprocess.run(['unzip', zip_location], check=True)
...
def download_file(url: str, checksum: str) -> str:
    try:
        ...
        # Check the checksum
        checksum_result = subprocess.run(['sha256sum', destination], capture_output=True, text=True, check=True)
        file_checksum = checksum_result.stdout.split()[0]
        print(f"Checksum for {destination}: {file_checksum}")
        
        if file_checksum != checksum:
            print(f"Error: Checksum mismatch for {destination}. Expected {checksum}, but got {file_checksum}. The downloaded file may be corrupted or tampered with.")
            sys.exit(1)
        ...
        if "CIRCUITPY" in mount_point:
            ...
            if os.path.exists(boot_out_path):
                with open(boot_out_path, 'r') as file:
                    content = file.read()
                    board_id_line = next((line for line in content.splitlines() if "Board ID:" in line), None)
                ...
                if board_id not in VALID_BOARD_IDS:
                    print(f"Skipping: The board ID {board_id} is not in the list of valid board IDs: {VALID_BOARD_IDS}")
                    continue
                
                for file_name in ["code.py", "settings.toml"]:
                    copy_file(
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Error: DEVICE_DRIVER_NAME is not set. Please supply it as the first argument.")
        sys.exit(1)
